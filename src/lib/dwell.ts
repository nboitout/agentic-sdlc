...
//
// A single page view can emit several `page_leave` rows (one per
// background/flush — see VisitTracker). For sums that's fine, but for
// averages it deflates the result (many small fragments instead of one
// visit). This collapses fragments back into one dwell value per visit.
//
// A visit = readerId + sessionId + page; fragments share all three and are
// summed. Rows with no sessionId are kept as their own visit.
