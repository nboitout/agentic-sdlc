Create a single horizontal sprite strip for the Codex app digital pet `muggy-cactus` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: sleepy cactus debugger with a mug; compact digital pet; tiny chibi cactus with a cozy coding vibe; half-lidded eyes; small ceramic mug held close; simplified cactus spikes; thick dark outline; limited desert green palette; pixel-art-adjacent house style; no text; no scenery.
